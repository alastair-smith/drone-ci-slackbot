...
raw_jobs = {
    ...
    "checksum dependencies lambda layer": {
        "image": images["aws cli"],
        "environment": build_variables,
        "commands": [
            "cd app",
            "LAYER_HASH=$(sha1sum package-lock.json build-layer.sh | sha1sum | awk '{ print $1 }')",
            'echo "LAYER_HASH = $LAYER_HASH"',
            'LAYER_ZIP_OBJECT_NAME="$DRONE_REPO_NAME/dependencies-layer/$LAYER_HASH.zip"',
    ...
    "upload dependencies lambda layer": {
        "image": images["aws cli"],
        "environment": build_variables,
        "depends_on": ["build dependencies lambda layer"],
        "commands": [
            "cd app",
            """if [ -f ".run-build" ]; then
